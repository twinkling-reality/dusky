...
describe("schema shapes generated by common TypeScript validators", () => {
  it("collects a required primitive declared through root allOf", () => {
    const composed = tool({
      inputSchema: {
        allOf: [
          {
    ...
    expect(parameters(composed).map((param) => [param.name, param.kind, param.required])).toEqual([
      ["query", "text", true],
    ]);
    expect(isOperable(composed)).toBe(true);
  });

  it("collects a required primitive from a root local object reference", () => {
    ...
    expect(parameters(referenced).map((param) => [param.name, param.kind, param.required])).toEqual(
      [["query", "text", true]],
    );
    expect(isOperable(referenced)).toBe(true);
  });

  it("never treats ambiguous or non-primitive root composition as an empty operable schema", () => {
    const nonPrimitive = tool({
      inputSchema: {
        allOf: [
          {
            type: "object",
    ...
    expect(isOperable(nonPrimitive)).toBe(false);
    expect(isOperable(ambiguous)).toBe(false);
  });

  it("accepts one Display primitive combined with null", () => {
    const nullable = tool({
      inputSchema: {
        type: "object",
        properties: {
    ...
    expect(parameters(nullable).map((p) => p.kind)).toEqual(["text", "number", "boolean"]);
    expect(isOperable(nullable)).toBe(true);
  });

  it("accepts the same nullable primitive shape expressed through anyOf or oneOf", () => {
    const nullable = tool({
      inputSchema: {
        type: "object",
        properties: {
          note: { anyOf: [{ type: "string" }, { type: "null" }] },
          count: { oneOf: [{ type: "null" }, { type: "number" }] },
        },
        required: ["note", "count"],
      },
    });

    expect(parameters(nullable).map((p) => p.kind)).toEqual(["text", "number"]);
    expect(isOperable(nullable)).toBe(true);
  });

  it("preserves an enum inside a nullable union", () => {
    const nullable = tool({
      inputSchema: {
        type: "object",
        properties: {
          mode: { oneOf: [{ type: "string", enum: ["safe"] }, { type: "null" }] },
        },
        required: ["mode"],
      },
    });
    const param = parameters(nullable)[0];
    expect(param?.kind).toBe("enum");
    if (!param) throw new Error("expected a parameter");
    expect(valueForParam("safe", param)).toBe("safe");
    expect(valueForParam("not-declared", param)).toBeUndefined();
  });

  it("refuses a fractional value for an integer schema", () => {
    const integer = tool({
      inputSchema: {
        type: "object",
    ...
    const param = parameters(integer)[0];
    if (!param) throw new Error("expected a parameter");
    expect(valueForParam("4", param)).toBe(4);
    expect(valueForParam("1.5", param)).toBeUndefined();
  });

  it("still refuses unions with more than one non-null primitive", () => {
    const ambiguous = tool({
      inputSchema: {
        type: "object",
    ...
    expect(parameters(ambiguous)[0]?.kind).toBe("unsupported");
    expect(isOperable(ambiguous)).toBe(false);
  });

  it("treats a primitive const as a one-value enum", () => {
    const fixed = tool({
      inputSchema: {
        type: "object",
        properties: { mode: { type: "string", const: "compact" } },
        required: ["mode"],
      },
    });
    const param = parameters(fixed)[0];
    expect(param?.kind).toBe("enum");
    if (!param) throw new Error("expected a parameter");
    const frame = paramFrame("Source", fixed, param);
    if (frame.kind !== "choose") throw new Error("expected a choice frame");
    expect(frame.choices.map((choice) => choice.id)).toEqual(["compact", "__cancel"]);
  });

  it("resolves a local definition reference to a string enum", () => {
    const referenced = tool({
      inputSchema: {
        type: "object",
        $defs: {
          mode: { type: "string", enum: ["quiet", "bright"] },
        },
    ...
    const param = parameters(referenced)[0];
    expect(param?.kind).toBe("enum");
    expect(isOperable(referenced)).toBe(true);
    if (!param) throw new Error("expected a parameter");
    const frame = paramFrame("Source", referenced, param);
    if (frame.kind !== "choose") throw new Error("expected a choice frame");
    expect(frame.choices.map((choice) => choice.id)).toContain("bright");
  });

  it("flattens a trivial allOf intersection of string and enum", () => {
    const intersected = tool({
      inputSchema: {
        type: "object",
        properties: {
          mode: { allOf: [{ type: "string" }, { enum: ["quiet", "bright"] }] },
        },
        required: ["mode"],
      },
    });

    expect(parameters(intersected)[0]?.kind).toBe("enum");
    expect(isOperable(intersected)).toBe(true);
  });

  it("treats provider-enforced string constraints inside allOf like a direct string schema", () => {
    const constrained = tool({
      inputSchema: {
        type: "object",
        properties: {
          query: { allOf: [{ type: "string", minLength: 1, pattern: "\\S" }] },
        },
        required: ["query"],
      },
    });

    expect(parameters(constrained)[0]?.kind).toBe("text");
    expect(isOperable(constrained)).toBe(true);
  });

  it("keeps a primitive reference operable when another allOf branch only describes it", () => {
    const described = tool({
      inputSchema: {
        ...
        properties: {
          query: {
            allOf: [{ $ref: "#/$defs/query" }, { description: "What should be found?" }],
          },
        },
        required: ["query"],
      },
    ...
    const param = parameters(described)[0];
    expect(param?.kind).toBe("text");
    expect(param?.description).toBe("What should be found?");
    expect(isOperable(described)).toBe(true);
  });

  it("still rejects an object branch hidden inside property allOf", () => {
    const nested = tool({
      inputSchema: {
        type: "object",
        properties: {
          settings: { allOf: [{ type: "object", properties: { mode: { type: "string" } } }] },
        },
        required: ["settings"],
      },
    });

    expect(parameters(nested)[0]?.kind).toBe("unsupported");
    expect(isOperable(nested)).toBe(false);
  });

  it("keeps object, array and object references unsupported", () => {
    const unsupported = tool({
      inputSchema: {
        type: "object",
        definitions: { settings: { type: "object", properties: { mode: { type: "string" } } } },
        properties: {
          directObject: { type: "object" },
          directArray: { type: "array", items: { type: "string" } },
          referencedObject: { $ref: "#/definitions/settings" },
          remote: { $ref: "https://schemas.test/value.json" },
        },
        required: ["directObject", "directArray", "referencedObject", "remote"],
      },
    });

    expect(parameters(unsupported).map((param) => param.kind)).toEqual([
      "unsupported",
      "unsupported",
      "unsupported",
      "unsupported",
    ]);
    expect(isOperable(unsupported)).toBe(false);
  });
});
...
describe("display constraints", () => {
  ...
  it("refuses a required argument that has no declared property schema", () => {
    const incomplete = tool({
      inputSchema: {
        type: "object",
        properties: {},
        required: ["missing_declaration"],
      },
    });

    expect(parameters(incomplete)).toEqual([
      expect.objectContaining({ name: "missing_declaration", required: true, kind: "unsupported" }),
    ]);
    expect(isOperable(incomplete)).toBe(false);
  });

  it("does not emit provider values that collide with Display control ids", () => {
    const colliding = tool({
      inputSchema: {
        type: "object",
        properties: { action: { type: "string", enum: ["__cancel"] } },
        required: ["action"],
      },
    });

    expect(parameters(colliding)[0]?.kind).toBe("unsupported");
    expect(isOperable(colliding)).toBe(false);
  });
});
...
describe("results become the next frame's choices", () => {
  ...
  it("skips earlier arrays whose elements are not candidate-shaped", () => {
    const raw = JSON.stringify({
      warnings: [{ code: "stale", message: "May be delayed" }],
      meta: ["cached"],
      departures: [{ flight_id: "BA117", title: "London to New York" }],
    });
    expect(candidatesFromResult(raw)).toEqual([
      { id: "BA117", label: "London to New York", meta: undefined },
    ]);
  });

  it("does not turn an explicit failed result into choices", () => {
    const raw = JSON.stringify({
      ok: false,
      error: "Lookup failed",
...
describe("the result envelope the protocol itself defines", () => {
  ...
  it("unwraps JSON choices carried in a text content block", () => {
    const raw = JSON.stringify({
      content: [
        {
          type: "text",
          text: JSON.stringify({
            matches: [{ uid: "shade-7", label: "Courtyard canopy", status: "healthy" }],
          }),
        },
      ],
    });
    expect(candidatesFromResult(raw)).toEqual([
      { id: "shade-7", label: "Courtyard canopy", meta: "healthy" },
    ]);
  });

  it("unwraps scalar facts carried as JSON in a text content block", () => {
    const raw = JSON.stringify({
      content: [
        {
          type: "text",
          text: JSON.stringify({ reference: "AB12", party_size: 4, confirmed: true }),
        },
      ],
    });

    expect(factsFromResult(raw)).toEqual([
      { label: "Reference", value: "AB12" },
      { label: "Party size", value: "4" },
      { label: "Confirmed", value: "Yes" },
    ]);
  });

  it("does not turn facts inside an explicit negative content envelope into success detail", () => {
    const raw = JSON.stringify({
      content: [
        {
          type: "text",
          text: JSON.stringify({ ok: false, reference: "AB12", message: "Not booked" }),
        },
      ],
    });

    expect(outcomeFromResult(raw).ok).toBe(false);
    expect(factsFromResult(raw)).toEqual([]);
  });

  it("keeps oversized and unreadable content-envelope facts inert", () => {
    const oversized = JSON.stringify({
      content: [
        { type: "text", text: JSON.stringify({ reference: "A".repeat(MAX_RESULT_CHARS) }) },
      ],
    });
    const unreadable = JSON.stringify({ content: [{ type: "text", text: "{not json" }] });

    expect(factsFromResult(oversized)).toEqual([]);
    expect(factsFromResult(unreadable)).toEqual([]);
  });
...
describe("failures a site spelled its own way", () => {
  ...
  it("reads the protocol's isError signal and never turns its content into choices", () => {
    const raw = JSON.stringify({
      isError: true,
      content: [
        {
          type: "text",
          text: JSON.stringify({ results: [{ id: "stale-1", name: "Stale result" }] }),
        },
      ],
    });
  ...
  it("reads failure and candidates from protocol structured content", () => {
    const failed = JSON.stringify({
      isError: false,
      structuredContent: { ok: false, error: "Denied" },
    });
    expect(outcomeFromResult(failed).ok).toBe(false);

    const choices = JSON.stringify({
      isError: false,
      structuredContent: { matches: [{ uid: "route-7", label: "Evening route" }] },
    });
    expect(candidatesFromResult(choices)).toEqual([
      { id: "route-7", label: "Evening route", meta: undefined },
    ]);
  });
...
describe("bounded shareable result projections", () => {
  ...
  it("does not retain protocol envelope metadata as transferable data", () => {
    const projections = shareableProjectionsFromResult(
      JSON.stringify({
        isError: false,
        content: [{ type: "text", text: "Readable fallback" }],
        structuredContent: { reference: "R-7" },
      }),
    );
    expect(projections).toContainEqual(
      expect.objectContaining({ location: "/structuredContent/reference", value: "R-7" }),
    );
    expect(projections.some((projection) => projection.value === false)).toBe(false);
    expect(projections.some((projection) => projection.value === "text")).toBe(false);
  });

  it("unwraps content-envelope JSON scalars into bounded field projections", () => {
    const raw = JSON.stringify({
      content: [
        {
          type: "text",
          text: JSON.stringify({ reference: "R-9", seats: 2, outside: false }),
        },
      ],
    });
    const projections = shareableProjectionsFromResult(raw);

    expect(factsFromResult(raw).map((fact) => fact.value)).toEqual(["R-9", "2", "No"]);
    expect(projections).toContainEqual(
      expect.objectContaining({ location: "/content/0/text/reference", value: "R-9" }),
    );
    expect(projections).toContainEqual(
      expect.objectContaining({ location: "/content/0/text/seats", value: 2 }),
    );
    expect(projections).toContainEqual(
      expect.objectContaining({ location: "/content/0/text/outside", value: false }),
    );
  });

  it("offers no transfer fields from an explicit negative content envelope", () => {
    const raw = JSON.stringify({
      content: [
        {
          type: "text",
          text: JSON.stringify({ ok: false, reference: "R-9", message: "Not available" }),
        },
      ],
    });

    expect(outcomeFromResult(raw).ok).toBe(false);
    expect(shareableProjectionsFromResult(raw)).toEqual([]);
  });

  it("sanitizes provider-written keys before they become visible labels", () => {
    const facts = factsFromResult(JSON.stringify({ "status\n__share": "ready" }));
    expect(facts).toEqual([{ label: "Status share", value: "ready" }]);
    expect(facts[0]?.label).not.toContain("\n");
  });
